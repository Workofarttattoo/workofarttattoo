...
Idempotent. Expects optimized WEBP assets in:
  artists/katelyn-cole/piercing-portfolio/

Images are routed by verified placement filename. The Katelyn profile targets
explicit portfolio block markers so a repeated heading elsewhere on the page
cannot cause ear/facial/body photos to overwrite the wrong gallery.
"""
...
def alt_for(path: Path) -> str:
    ...
    labels = [
        ("nostril", "Nostril piercing"), ("septum", "Septum piercing"),
        ("earlobe", "Ear lobe piercing"), ("helix", "Helix piercing"),
        ("ear-cartilage", "Ear cartilage piercing"), ("upper-ear", "Upper ear piercing"),
        ("curated-ear", "Curated ear piercings"), ("navel", "Navel piercing"),
        ("lip-labret", "Lip and labret piercing"), ("lip-nostril", "Facial piercing"),
        ("cheek", "Cheek piercing"), ("tongue", "Tongue piercing appointment"),
        ("ear-piercing-process", "Ear piercing appointment"),
        ("piercing-process", "Piercing appointment"),
        ("piercing-client", "Piercing client"),
        ("portfolio-mixed", "Piercing portfolio work"),
    ]
...
def img_tag(path: Path) -> str:
    ...
    return (f'<figure class="overflow-hidden border border-outline-variant/20 bg-surface-container">'
            ...
            f'class="w-full h-full object-cover object-center" width="600" height="600">'</n            f'</figure>')
...
def replace_dense_grid_in_marked_block(doc: str, marker_text: str, paths: list[Path]) -> str:
    if not paths:
        return doc
...
def update_katelyn_profile() -> bool:
    ...
    face_body = assets(("katelyn-nostril-", "katelyn-septum-", "katelyn-lip-labret-", "katelyn-lip-nostril-", "katelyn-cheek-", "katelyn-navel-", "katelyn-tongue-"))
    ...
    doc = replace_dense_grid_in_marked_block(doc, "BLOCK 1: Anatomical Ear Curation", ear)
    doc = replace_dense_grid_in_marked_block(doc, "BLOCK 2: Facial & Body Piercing", face_body)
    doc = replace_dense_grid_in_marked_block(doc, "BLOCK 3: Luxury Jewelry", studio)
    ...
    doc = re.sub(r'Explore meticulously documented piercing case studies — ear curation, facial and body work, and luxury jewelry styling\.',
                 "Real piercing work from the studio, organized by placement so you can see Katelyn's actual client work.", doc, count=1)
    doc = doc.replace(">Scroll for 100+ Live Case Studies<", ">Real Piercing Work<", 1)
    if doc != old:
        path.write_text(doc, encoding="utf-8"); return True
    return False
...
def insert_before_close(path: Path, section: str) -> bool:
    ...
    if MARKER_START in doc:
        doc2 = re.sub(re.escape(MARKER_START) + r".*?" + re.escape(MARKER_END), section, doc, flags=re.S)
    elif "</main>" in doc: doc2 = doc.replace("</main>", section + "\n</main>", 1)
    elif "</body>" in doc: doc2 = doc.replace("</body>", section + "\n</body>", 1)
    else: return False
    if doc2 != doc: path.write_text(doc2, encoding="utf-8"); return True
    return False
...
<div class="max-w-6xl mx-auto"><div class="max-w-3xl mb-8">
...
<p class="text-body-lg font-body-lg text-on-surface-variant mt-3">{html.escape(intro)}</p></div>
{grid(paths)}
...
def update_main_piercing_guide() -> bool:
    ...
    if not path.is_file(): return False
    representative=[]
    for pref in ("katelyn-helix-", "katelyn-curated-ear-", "katelyn-nostril-", "katelyn-septum-", "katelyn-lip-labret-", "katelyn-navel-", "katelyn-earlobe-", "katelyn-cheek-", "katelyn-piercing-process-", "katelyn-professional-piercer-"):
        found=assets(pref)
        if found: representative.extend(found[:2])
    sec=section_for("Real Piercing Work by Katelyn", "These are real clients from Work of Art Tattoo & Piercing — not stock photos. Placement and jewelry are planned around the client's anatomy and the piercing being performed.", representative)
...
def update_matching_existing_pages()->list[str]:
    changed=[]; skip_roots={"artists_build","artists_raw",".git","node_modules"}
    for path in ROOT.rglob("*.html"):
        if any(part in skip_roots for part in path.parts): continue
        ...
        for needles,title,intro,prefixes in PAGE_RULES:
            if any(n in rel for n in needles):
                pics=assets(prefixes); sec=section_for(title,intro,pics)
                if sec and insert_before_close(path,sec): changed.append(rel)
                break
    return changed

def main()->int:
    if not ASSET_DIR.is_dir(): raise SystemExit(f"Missing asset directory: {ASSET_DIR}")
    pics=list(ASSET_DIR.glob("*.webp"))
    if not pics: raise SystemExit("No Katelyn piercing WEBP assets found")
    changed=[]
    if update_katelyn_profile(): changed.append("artists_build/katelyn-cole.html")
    if update_main_piercing_guide(): changed.append("best_piercing_shop_las_vegas_updated_jewelry_standards/code.html")
    changed.extend(update_matching_existing_pages())
    print(f"Katelyn piercing assets found: {len(pics)}")
    print("Updated files:")
    for rel in sorted(set(changed)): print(f"  - {rel}")
    return 0
