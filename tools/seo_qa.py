...
def validate_sitewide_files(failures: list[str]) -> None:
    ...
    if not index_path.is_file():
        ...
        if "<sitemapindex" not in index_text:
            failures.append("sitemap.xml: expected sitemap index")
        for loc in re.findall(r"<loc>(.*?)</loc>", index_text):
            assert_host(loc, "sitemap.xml", failures)
        expected_index_loc = f"{CANONICAL_HOST.rstrip('/')}/sitemap-static-pages.xml"
        if expected_index_loc not in index_text:
            failures.append("sitemap.xml: missing sitemap-static-pages.xml child reference")

    static_path = ROOT / "sitemap-static-pages.xml"
    if not static_path.is_file():
        failures.append("sitemap-static-pages.xml: missing")
        return
    text = static_path.read_text(encoding="utf-8", errors="ignore")
    if "<urlset" not in text:
        failures.append("sitemap-static-pages.xml: expected urlset")
    for loc in re.findall(r"<loc>(.*?)</loc>", text):
        assert_host(loc, "sitemap-static-pages.xml", failures)
    for slug in load_merge_slugs():
        if f"/{slug}/" in text:
            failures.append(f"sitemap-static-pages.xml: MERGE URL still appears in sitemap: /{slug}/")
    for slug in sorted(MERGED_GEO_SLUGS | RETIRE_OVERLAP_SLUGS):
        if f"/{slug}/" in text:
            failures.append(f"sitemap-static-pages.xml: retired geo/overlap URL still appears in sitemap: /{slug}/")
    if WEEKLY_PROMO_URL_RE.search(text):
        failures.append("sitemap-static-pages.xml: weekly/date-based piercing specials URL found")
    for slug in PROMO_SLUGS:
        if slug and f"/{slug}/" not in text:
            failures.append(f"sitemap-static-pages.xml: permanent promotion URL missing from sitemap: /{slug}/")
...
def published_routes() -> set[str]:
    path = ROOT / "sitemap-static-pages.xml"
    if not path.is_file():
