...
TARGETS = (
    ...
    ROOT / "artists" / "katelyn-cole" / "code.html",
)
BOOK = "https://jim.com/a/katelyn-delano-rose-morg"

BIO_BLOCK = """<h2 class="text-headline-lg font-headline-lg mb-12">About Katelyn Cole</h2>
...
def polish_html(s: str) -> str:
    replacements = {
        ...
        "https://www.workofarttattoo.com/best_fine_line_tattoos_in_vegas_ultimate_authority_guide/best-tattoo-las-vegas-custom-sleeve-by-master-artist.webp": "https://www.workofarttattoo.com/artists/katelyn-cole/piercing-portfolio/katelyn-helix-piercing-02.webp",
    ...
    s = re.sub(
        r"(?is)<h2[^>]*>The Definitive Bio of Katelyn Cole</h2>.*?(?=<section\b[^>]*id=\"piercing-minors\")",
        BIO_BLOCK,
        s,
        count=1,
    )
    s = re.sub(
        r"(?is)(<span class=\"text-label-caps font-label-caps text-secondary block mb-4 uppercase\">The Artist &amp; The Authority</span>\s*)?"
        r"<h2[^>]*>About Katelyn Cole</h2>.*?(?=<section\b[^>]*id=\"piercing-minors\")",
        r'\1' + BIO_BLOCK,
        s,
        count=1,
    )
    ...
    s = s.replace(
        'href="/appointments/">Book Piercing Appointment</a>',
        f'href="{BOOK}" target="_blank" rel="noopener noreferrer">View Prices &amp; Book Katelyn</a>',
    )
    s = s.replace(
        'href="/appointments/">Book a piercing appointment</a>',
        f'href="{BOOK}" target="_blank" rel="noopener noreferrer">View prices &amp; book Katelyn</a>',
    )
...
def main() -> int:
    ...
    print(f"Done: {changed} Katelyn profile(s) updated")
    return 0
