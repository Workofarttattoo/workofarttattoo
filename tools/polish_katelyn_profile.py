...
def main():
    ...
    replacements={
      ...
      'https://workofarttattoo.com/best_fine_line_tattoos_in_vegas_ultimate_authority_guide/best-tattoo-las-vegas-custom-sleeve-by-master-artist.webp':f'https://workofarttattoo.com/artists/katelyn-cole/piercing-portfolio/katelyn-helix-piercing-02.webp',
    ...
    # Rewrite inflated bio as grounded first-hand studio information.
    s=re.sub(r'(?is)<h2 class="text-headline-lg font-headline-lg mb-12">The Definitive Bio of Katelyn Cole</h2>.*?</div>\s*</div>\s*<section class="py-section-gap px-margin-desktop bg-surface border-y',
    '''<h2 class="text-headline-lg font-headline-lg mb-12">About Katelyn Cole</h2>
...
<section class="py-section-gap px-margin-desktop bg-surface border-y''',s,count=1)
    # Decorative icon-font words do not need to exist as textual content.
    s=re.sub(r'<span class="material-symbols-outlined[^>]*">(?:earbuds|face_5|vaccines)</span>','',s)
    # Direct Katelyn CTAs should use her live pricing/calendar source.
    ...
    if s!=old:P.write_text(s,encoding='utf-8');print('updated',P);return 0
